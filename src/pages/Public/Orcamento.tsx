        ...
        <div className="flex flex-col items-center">
          <div className="relative mb-6">
             <img 
               src="/image.png" 
               alt="JL Arte Camisas" 
               className="h-16 md:h-20 object-contain drop-shadow-[0_0_15px_rgba(250,204,21,0.3)]"
             />
          </div>
          ...
          {steps.map((step) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center group relative flex-1">
                {step.id < 8 && (
                  <div className={`absolute top-6 left-[60%] right-[-40%] h-[2px] transition-all duration-500 ${
                    isCompleted ? 'bg-yellow-500' : 'bg-slate-700'
                  }`} />
                )}
                <div className={`w-7 h-7 rounded-2xl flex items-center justify-center border-2 transition-all duration-500 relative z-10 ${
                  isActive ? 'bg-yellow-500 border-yellow-500 text-white scale-110 step-indicator-active' :
                  isCompleted ? 'bg-yellow-500 border-yellow-500 text-white' :
                  'bg-slate-800 border-slate-700 text-slate-500'
                }`}>
                  {isCompleted ? <Check className="w-6 h-6" /> : <Icon className="w-5 h-5" />}
                </div>
                <span className={`mt-3 text-[10px] font-bold uppercase tracking-wide transition-colors ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {opcoes.quantidade.map(opt => (
                <button
                  ...
                  className={`option-card p-4 md:p-5 rounded-2xl border-2 text-left transition-all ${
                    formData.quantidade === opt.label ? 'option-card-selected' : 'bg-slate-800/30 border-slate-700'
                  }`}
                >
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mb-4 ${
                    formData.quantidade === opt.label ? 'bg-yellow-500 border-yellow-500' : 'border-slate-600'
                  }`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {opcoes.prazo.map(opt => (
                <button
                  ...
                  className={`option-card p-4 md:p-5 rounded-2xl border-2 text-left transition-all ${
                    formData.prazo === opt.label ? 'option-card-selected' : 'bg-slate-800/30 border-slate-700'
                  }`}
                >
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mb-4 ${
                    formData.prazo === opt.label ? 'bg-yellow-500 border-yellow-500' : 'border-slate-600'
                  }`}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              {opcoes.modelo.map(opt => (
                <button
                  ...
                  className={`option-card p-4 md:p-6 rounded-2xl border-2 transition-all text-center flex flex-col items-center gap-4 ${
                    formData.modelo === opt.label ? 'option-card-selected' : 'bg-slate-800/30 border-slate-700'
                  }`}
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {opcoes.estampa.map(opt => (
                <button
                  ...
                  className={`option-card p-8 md:p-12 rounded-[40px] border-2 text-center transition-all ${
                    formData.estampa === opt.label ? 'option-card-selected' : 'bg-slate-800/30 border-slate-700'
                  }`}
                >
                  <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center mx-auto mb-6 ${
                    formData.estampa === opt.label ? 'bg-yellow-500 border-yellow-500' : 'border-slate-600'
                  }`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {opcoes.tecido.map(opt => (
                <button
                  ...
                  className={`option-card p-4 md:p-5 rounded-2xl border-2 text-left transition-all ${
                    formData.tecido === opt.label ? 'option-card-selected' : 'bg-slate-800/30 border-slate-700'
                  }`}
                >
                  <span className="font-black text-white text-sm md:text-base uppercase italic">{opt.label}</span>
                </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               {opcoes.adicional.map(opt => (
                 <button
                  ...
                  className={`option-card p-6 rounded-2xl border-2 transition-all flex items-center justify-between ${
                    formData.adicionais.includes(opt.label) ? 'option-card-selected' : 'bg-slate-800/30 border-slate-700'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center ${
                      formData.adicionais.includes(opt.label) ? 'bg-yellow-500 border-yellow-500' : 'border-slate-600'
                    }`}>
